...
import com.github.xzlink.rtmp.RtmpClient;
import com.github.xzlink.rtmp.amf.*;
import com.github.xzlink.rtmp.io.packets.*;
...
public class RtmpConnection implements RtmpClient, PacketRxHandler, ThreadController {
    ...
    public void pushMessage(byte[] data,boolean isPushDataFrame) {
        ...
        rtmpHeader1.setMessageType(RtmpHeader.MessageType.VIDEO);
        rtmpHeader1.setChunkStreamId(ChunkStreamInfo.RTMP_STREAM_CHANNEL);
        rtmpHeader1.setChunkType(RtmpHeader.ChunkType.TYPE_0_FULL);
        rtmpHeader1.setMessageStreamId(currentStreamId);
        rtmpHeader1.setAbsoluteTimestamp((int)System.currentTimeMillis());
        Video data1 = new Video(rtmpHeader1);
        data1.setData(data);

        if(isPushDataFrame){
            AmfMap amfMap = new AmfMap();
            amfMap.setProperty("duration",0);
            amfMap.setProperty("width",1280);
            amfMap.setProperty("height",720);
            amfMap.setProperty("videodatarate",0);
            amfMap.setProperty("videocodecid",7);
            amfMap.setProperty("title","Media Presentation");
//            amfMap.setProperty("encoder","flv");//Lavf57.51.103
            amfMap.setProperty("filesize",0);

            SetDataFrame setDataFrame = new SetDataFrame(amfMap);
            setDataFrame.getHeader().setChunkStreamId(4);
            setDataFrame.getHeader().setMessageStreamId(currentStreamId);
            writeThread.send(setDataFrame,data1);

        }else{
            writeThread.send(data1);
        }
    }
    ...
    private void rtmpCreateStream() {
        ...
        L.d("rtmpCreateStream(): Sending releaseStream command...");
        ...
        writeThread.send(releaseStream,fcPublish,createStream);
    }
...
//            amfMap.setProperty("videocodecid",7);
//            amfMap.setProperty("title","Media Presentation");
//            amfMap.setProperty("encoder","Lavf57.51.103");
//            amfMap.setProperty("filesize",0);
//
//            SetDataFrame setDataFrame = new SetDataFrame(amfMap);
//            setDataFrame.getHeader().setChunkStreamId(4);
//            setDataFrame.getHeader().setMessageStreamId(currentStreamId);
//            writeThread.send(setDataFrame);

//            byte[] data = new byte[]{1};
//            RtmpHeader rtmpHeader1 = new RtmpHeader();
//            rtmpHeader1.setMessageType(RtmpHeader.MessageType.VIDEO);
//            rtmpHeader1.setChunkStreamId(6);
//            rtmpHeader1.setChunkType(RtmpHeader.ChunkType.TYPE_0_FULL);
//            rtmpHeader1.setMessageStreamId(currentStreamId);
//            Video data1 = new Video(rtmpHeader1);
//            data1.setData(data);
...
//
//            byte[] data = new byte[1493];
//            while(input.read(data)!=-1){
//                rtmpHeader1.setMessageType(RtmpHeader.MessageType.DATA_AMF0);
//                rtmpHeader1.setChunkStreamId(ChunkStreamInfo.RTMP_STREAM_CHANNEL);
//                rtmpHeader1.setChunkType(RtmpHeader.ChunkType.TYPE_0_FULL);
//                rtmpHeader1.setMessageStreamId(currentStreamId);
//                data1 = new Video(rtmpHeader1);
//                data1.setData(data);
//                writeThread.send(data1);
//            }
        }else {
